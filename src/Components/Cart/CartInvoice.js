...
export const CartInvoice = ({ items }) => {
  ...
  const totalPrice = items.reduce((acc, item) => acc + item.attributes.price, 0);
  const originalPrice = items.reduce((acc, item) => acc + item.attributes.actualprice, 0);
  const totalDeliveryCharge = items.reduce((acc, item) => acc + item.attributes.deliveryCharges, 0);
  const isFreeDelivery = totalPrice > 700;
  const discountPrice = originalPrice - totalPrice;
  const finalPrice = totalPrice + (isFreeDelivery ? 0 : totalDeliveryCharge);
